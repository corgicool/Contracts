...
contract CorgiTreatToken is Context, IERC20, Ownable {
    using SafeMath for uint256;
    using Address for address;

    string private constant NAME = "bCORGI TREAT";
    string private constant SYMBOL = "TRT";
    ...
    uint256 private constant ACTUAL_TOTAL = 20_000 * 1e18; //20K
    ...
    uint256 public holderTaxAlloc;
    uint256 public marketingTaxAlloc;
...
contract CorgiTreatFarm is Ownable {
    using SafeMath for uint256;
    ...
    struct UserInfo {
        ...
        //
        // We do some fancy math here. Basically, any point in time, the amount of TREAT tokens
        // entitled to a user but is pending to be distributed is:
        //
        //   pending reward = (user.amount * pool.accTreatPerShare) - user.rewardDebt
        //
        // Whenever a user deposits or withdraws LP tokens to a pool. Here's what happens:
        //   1. The pool's `accTreatPerShare` (and `lastRewardBlock`) gets updated.
    ...
    struct PoolInfo {
        ...
        uint256 allocPoint; // How many allocation points assigned to this pool. TREAT tokens to distribute per block.
        uint256 lastRewardBlock; // Last block number that TREAT tokens distribution occurs.
        uint256 accTreatPerShare; // Accumulated TREAT tokens per share, times 1e12. See below.
    }

    CorgiTreatToken public immutable treat; // The TREAT ERC-20 Token.
    uint256 private treatPerBlock; // TREAT tokens distributed per block. Use getTreatPerBlock() to get the updated reward.

    PoolInfo[] public poolInfo; // Info of each pool.
    mapping(uint256 => mapping(address => UserInfo)) public userInfo; // Info of each user that stakes LP tokens.
    uint256 public totalAllocPoint; // Total allocation points. Must be the sum of all allocation points in all pools.
    uint256 public startBlock; // The block number when TREAT token mining starts.

    uint256 public blockRewardUpdateCycle = 1 days; // The cycle in which the treatPerBlock gets updated.
    uint256 public blockRewardLastUpdateTime = block.timestamp; // The timestamp when the block treatPerBlock was last updated.
    uint256 public blocksPerDay = 28750; // The estimated number of mined blocks per day.
    uint256 public blockRewardPercentage = 1; // The percentage used for treatPerBlock calculation.
    ...
    constructor(
        CorgiTreatToken _treat,
        uint256 _startBlock
    ) {
        require(address(_treat) != address(0), "TREAT address is invalid");
    ...
    modifier updateTreatPerBlock() {
        (uint256 blockReward, bool update) = getTreatPerBlock();
        if (update) {
            treatPerBlock = blockReward;
            blockRewardLastUpdateTime = block.timestamp;
        }
        _;
    }

    function getTreatPerBlock() public view returns (uint256, bool) {
        if (block.number < startBlock) {
            return (0, false);
        }

        uint256 poolReward = treat.balanceOf(address(this));
        if (poolReward == 0) {
            return (0, treatPerBlock != 0);
        }

        if (block.timestamp >= getTreatPerBlockUpdateTime() || treatPerBlock == 0) {
            return (poolReward.mul(blockRewardPercentage).div(100).div(blocksPerDay), true);
        }

        return (treatPerBlock, false);
    }

    function getTreatPerBlockUpdateTime() public view returns (uint256) {
    ...
    // Update the given pool's TREAT token allocation point. Can only be called by the owner.
    function set(
        uint256 _pid,
        uint256 _allocPoint,
        bool _withUpdate
    ...
    // View function to see pending TREAT tokens on frontend.
    function pendingRewards(uint256 _pid, address _user) external view returns (uint256) {
        ...
        if (block.number > pool.lastRewardBlock && lpSupply != 0) {
            ...
            (uint256 blockReward, ) = getTreatPerBlock();
            uint256 treatReward = multiplier.mul(blockReward).mul(pool.allocPoint).div(totalAllocPoint);
            accTreatPerShare = accTreatPerShare.add(treatReward.mul(1e12).div(lpSupply));
        }
    ...
    function updatePool(uint256 _pid) public updateTreatPerBlock {
        PoolInfo storage pool = poolInfo[_pid];
        ...
        uint256 multiplier = block.number.sub(pool.lastRewardBlock);
        uint256 treatReward = multiplier.mul(treatPerBlock).mul(pool.allocPoint).div(totalAllocPoint);

        // no minting is required, the contract should have TREAT token balance pre-allocated
        // accumulated TREAT per share is stored multiplied by 10^12 to allow small 'fractional' values
        pool.accTreatPerShare = pool.accTreatPerShare.add(treatReward.mul(1e12).div(lpSupply));
        pool.lastRewardBlock = block.number;
    }

    // Deposit LP tokens to TreatFarming for TREAT token allocation.
    function deposit(uint256 _pid, uint256 _amount) public {
        ...
        if (user.amount > 0) {
            uint256 pending = user.amount.mul(pool.accTreatPerShare).div(1e12).sub(user.rewardDebt);
            if (pending > 0) {
    ...
    // Withdraw LP tokens from TreatFarming
    ...
    // Safe TREAT token transfer function, just in case if
    // rounding error causes pool to not have enough TREAT tokens
    function safeTokenTransfer(address _to, uint256 _amount) internal {
        uint256 balance = treat.balanceOf(address(this));
        uint256 amount = _amount > balance ? balance : _amount;
        treat.transfer(_to, amount);
    }
