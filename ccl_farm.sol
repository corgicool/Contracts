...
contract TreatTokenWrapper {
	using SafeMath for uint256;
	IERC20 public treat;

	constructor(IERC20 _treatAddress) public {
		treat = IERC20(_treatAddress);
	}
	...
	function stake(uint256 amount) public {
		...
		treat.transferFrom(msg.sender, address(this), amount);
	}

	function withdraw(uint256 amount) public {
		_totalSupply = _totalSupply.sub(amount);
		_balances[msg.sender] = _balances[msg.sender].sub(amount);
		treat.transfer(msg.sender, amount);
	}
}

contract GenesisPool is TreatTokenWrapper, Ownable {
	ERC1155Tradable public ccl;
	...
	constructor(ERC1155Tradable _cclAddress, IERC20 _treatAddress) public TreatTokenWrapper(_treatAddress) {
		ccl = _cclAddress;
	}
	...
	// stake visibility is public as overriding TreatTokenWrapper's stake() function
	function stake(uint256 amount) public updateReward(msg.sender) {
		require(amount.add(balanceOf(msg.sender)) <= 10000000000, "Cannot stake more than 100 treat");
	...
	function redeem(uint256 card) public updateReward(msg.sender) {
		...
		require(ccl.totalSupply(card) < ccl.maxSupply(card), "Max cards minted");

		points[msg.sender] = points[msg.sender].sub(cards[card]);
		ccl.mint(msg.sender, card, 1, "");
